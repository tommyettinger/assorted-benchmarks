...
/** Holds the geometry, color, and texture information for drawing 2D sprites using {@link Batch}. A Spritz has a position and a
 * size given as width and height. The position is relative to the origin of the coordinate system specified via
 * {@link Batch#begin()} and the respective matrices. A Spritz is always rectangular and its position (x, y) are located in the
 * bottom left corner of that rectangle. A Spritz also has an origin around which rotations and scaling are performed (that is,
 * the origin is not modified by rotation and scaling). The origin is given relative to the bottom left corner of the Spritz, its
 * position.
 * @author mzechner
 * @author Nathan Sweet */
public class Spritz extends TextureRegion {
	...
	public Spritz(Texture texture, int srcWidth, int srcHeight) {
	 ...
	 * @param srcWidth The width of the texture region. May be negative to flip the sprite when drawn.
	 * @param srcHeight The height of the texture region. May be negative to flip the sprite when drawn. */
	public Spritz(Texture texture, int srcX, int srcY, int srcWidth, int srcHeight) {
	...
	public Spritz(TextureRegion region) {
		setRegion(region);
		setColor(1, 1, 1, 1);
	 ...
	 * @param srcWidth The width of the texture region. May be negative to flip the sprite when drawn.
	 * @param srcHeight The height of the texture region. May be negative to flip the sprite when drawn. */
	public Spritz(TextureRegion region, int srcX, int srcY, int srcWidth, int srcHeight) {
	...
	public Spritz(Spritz sprite) {
		set(sprite);
	}

	/** Make this sprite a copy in every way of the specified sprite */
	public void set (Spritz sprite) {
	...
	public float[] getVertices () {
		if (dirty) {
			...
			if (rotation != 0) {
				final float cos = (float) Math.cos(0.017453292519943295 * rotation);
				final float sin = (float) Math.sin(0.017453292519943295 * rotation);
	...
	public void setRegion (float u, float v, float u2, float v2) {
		...
		float[] vertices = Spritz.this.vertices;
		vertices[U1] = u;
		vertices[V1] = v2;
