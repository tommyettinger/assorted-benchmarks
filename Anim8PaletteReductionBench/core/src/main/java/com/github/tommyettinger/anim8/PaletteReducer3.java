...
//        }
     ...
     * to speed up construction using {@link #PaletteReducer3(int[], byte[])}. Editing this field is strongly
    ...
    protected final float[] paletteLightness = new float[256];
     ...
     * It is protected so that user code that extends PaletteReducer3 doesn't need to create its own buffers.
     */
    ...
    /**
     * If this PaletteReducer3 has already calculated a palette, you can use this to save the slightly-slow-to-compute
     ...
     * PaletteReducer3 with {@code new PaletteReducer3(intColors, PaletteReducer3.loadPreloadFile(theFile))}.
     ...
     * constructor with: {@code new PaletteReducer3(intColors, PaletteReducer3.loadPreloadFile(theFile))}, where intColors
     ...
     * @return a byte array that should have a length of exactly 32768, to be passed to {@link #PaletteReducer3(int[], byte[])}
     */
    ...
    /**
     * Constructs a default PaletteReducer3 that uses the "Snuggly" 255-color-plus-transparent palette.
     * Note that this uses a more-detailed and higher-quality metric than you would get by just specifying
     * {@code new PaletteReducer3(PaletteReducer3.SNUGGLY)}; this metric would be too slow to calculate at
     * runtime, but as pre-calculated data it works very well.
     */
    public PaletteReducer3() {
        exact(SNUGGLY, ENCODED_SNUGGLY);
    }

    /**
     * Constructs a PaletteReducer3 that uses the given array of RGBA8888 ints as a palette (see {@link #exact(int[])}
     * for more info).
     *
     * @param rgbaPalette an array of RGBA8888 ints to use as a palette
     */
    public PaletteReducer3(int[] rgbaPalette) {
        if(rgbaPalette == null)
        {
            exact(SNUGGLY, ENCODED_SNUGGLY);
            return;
    ...
    /**
     * Constructs a PaletteReducer3 that uses the given array of RGBA8888 ints as a palette (see
     * {@link #exact(int[], int)} for more info).
     *
     * @param rgbaPalette an array of RGBA8888 ints to use as a palette
     * @param limit how many int items to use from rgbaPalette (this always starts at index 0)
     */
    public PaletteReducer3(int[] rgbaPalette, int limit) {
        if(rgbaPalette == null)
        {
            exact(SNUGGLY, ENCODED_SNUGGLY);
            return;
    ...
    /**
     * Constructs a PaletteReducer3 that uses the given array of Color objects as a palette (see {@link #exact(Color[])}
     * for more info).
     *
     * @param colorPalette an array of Color objects to use as a palette
     */
    public PaletteReducer3(Color[] colorPalette) {
    ...
    /**
     * Constructs a PaletteReducer3 that uses the given array of Color objects as a palette (see
     * {@link #exact(Color[], int)} for more info).
     *
     * @param colorPalette an array of Color objects to use as a palette
     */
    public PaletteReducer3(Color[] colorPalette, int limit) {
        if(colorPalette == null)
        {
            exact(SNUGGLY, ENCODED_SNUGGLY);
            return;
    ...
    /**
     * Constructs a PaletteReducer3 that analyzes the given Pixmap for color count and frequency to generate a palette
     * (see {@link #analyze(Pixmap)} for more info).
     *
     * @param pixmap a Pixmap to analyze in detail to produce a palette
     */
    public PaletteReducer3(Pixmap pixmap) {
        if(pixmap == null)
        {
            exact(SNUGGLY, ENCODED_SNUGGLY);
            return;
    ...
    /**
     * Constructs a PaletteReducer3 that analyzes the given Pixmaps for color count and frequency to generate a palette
     * (see {@link #analyze(Array)} for more info).
     *
     * @param pixmaps an Array of Pixmap to analyze in detail to produce a palette
     */
    public PaletteReducer3(Array<Pixmap> pixmaps) {
        if(pixmaps == null)
        {
            exact(SNUGGLY, ENCODED_SNUGGLY);
            return;
     ...
     * You can use {@link #writePreloadFile(FileHandle)} to write the preload data for a given PaletteReducer3, and
     ...
     */
    public PaletteReducer3(int[] palette, byte[] preload)
    {
     ...
     */
    public PaletteReducer3(Pixmap pixmap, double threshold) {
...
//    }
     ...
     * PaletteReducer3 to change one, some, or all of these methods. The other difference methods call the 6-argument
...
//        return (rf * rf + gf * gf + bf * bf) * 0x1.4p17;
    ...
    /**
     * Resets the palette to the 256-color (including transparent) "Snuggly" palette. PaletteReducer3 already
    ...
    /**
     * Builds the palette information this PaletteReducer3 stores from the given array of RGBA8888 ints as a palette (see
     * {@link #exact(int[])} for more info) and an encoded byte array to use to look up pre-loaded color data. The
     * encoded byte array can be copied out of the {@link #paletteMapping} of an existing PaletteReducer3. There's
     * slightly more startup time spent when initially calling {@link #exact(int[])}, but it will produce the same
     * result. You can store the paletteMapping from that PaletteReducer3 once, however you want to store it, and send it
    ...
    /**
     * Builds the palette information this PaletteReducer3 stores from the Color objects in {@code colorPalette}, up to
     * 256 colors.
...
//    };
     ...
     * value but instead stores the palette info in this object; a PaletteReducer3 can be assigned to the
     ...
     * PaletteReducer3 can be assigned to the {@link PNG8#palette} or {@link AnimatedGif#palette} fields or can be used
...
//                    (OtherMath.barronSpline(totalB / (len<<1)+0.5f, 2f, 0.5f)-0.5f)*2f,
     ...
     * This doesn't return a value but instead stores the palette info in this object; a PaletteReducer3 can be assigned
...
//            }
     ...
     * colors. This doesn't return a value but instead stores the palette info in this object; a PaletteReducer3 can be
     ...
     * a value but instead stores the palette info in this object; a PaletteReducer3 can be assigned to the
...
//                    (OtherMath.barronSpline(totalB / (len<<1)+0.5f, 2f, 0.5f)-0.5f)*2f,
    ...
    /**
     * Modifies the given Pixmap so that it only uses colors present in this PaletteReducer3, dithering when it can by
    ...
    /**
     * Modifies the given Pixmap so it only uses colors present in this PaletteReducer3, without dithering. This produces
    ...
    /**
     * Modifies the given Pixmap so that it only uses colors present in this PaletteReducer3, dithering when it can with
        ...
        for (int y = 0; y < h; y++) {
            ...
            for (int px = 0; px < lineLen; px++) {
                ...
                if ((color & 0x80) == 0 && hasTransparent)
                    ...
                    er = Math.min(Math.max(( ( (PaletteReducer3.TRI_BLUE_NOISE  [(px & 63) | (y & 63) << 6] + 0.5f) + ((((px+1) * 0xC13FA9A902A6328FL + (y +1) * 0x91E10DA5C79E7B1DL) >>> 41) * 0x1p-16f - 0x1p+6f)) * strength) + (curErrorRed[px]), -limit), limit);
                    eg = Math.min(Math.max(( ( (PaletteReducer3.TRI_BLUE_NOISE_B[(px & 63) | (y & 63) << 6] + 0.5f) + ((((px+3) * 0xC13FA9A902A6328FL + (y -1) * 0x91E10DA5C79E7B1DL) >>> 41) * 0x1p-16f - 0x1p+6f)) * strength) + (curErrorGreen[px]), -limit), limit);
                    eb = Math.min(Math.max(( ( (PaletteReducer3.TRI_BLUE_NOISE_C[(px & 63) | (y & 63) << 6] + 0.5f) + ((((px+2) * 0xC13FA9A902A6328FL + (y -4) * 0x91E10DA5C79E7B1DL) >>> 41) * 0x1p-16f - 0x1p+6f)) * strength) + (curErrorBlue[px]), -limit), limit);
        ...
        for (int y = 0; y < h; y++) {
            ...
            for (int x = 0; x < lineLen; x++) {
                ...
                if ((color & 0x80) == 0 && hasTransparent)
                    ...
                    er = Math.min(Math.max(( ( (PaletteReducer3.TRI_BLUE_NOISE  [(x & 63) | (y & 63) << 6] + 0.5f) * blueStrength + ((((x+1) * 0xC13FA9A902A6328FL + (y+1) * 0x91E10DA5C79E7B1DL) >>> 41) * 0x1.4p-24f - 0x1.4p-2f) * strength)), -limit), limit) + (curErrorRed[x]);
                    eg = Math.min(Math.max(( ( (PaletteReducer3.TRI_BLUE_NOISE_B[(x & 63) | (y & 63) << 6] + 0.5f) * blueStrength + ((((x+3) * 0xC13FA9A902A6328FL + (y-1) * 0x91E10DA5C79E7B1DL) >>> 41) * 0x1.4p-24f - 0x1.4p-2f) * strength)), -limit), limit) + (curErrorGreen[x]);
                    eb = Math.min(Math.max(( ( (PaletteReducer3.TRI_BLUE_NOISE_C[(x & 63) | (y & 63) << 6] + 0.5f) * blueStrength + ((((x+2) * 0xC13FA9A902A6328FL + (y-4) * 0x91E10DA5C79E7B1DL) >>> 41) * 0x1.4p-24f - 0x1.4p-2f) * strength)), -limit), limit) + (curErrorBlue[x]);
...
//                    float pos = (PaletteReducer3.thresholdMatrix64[(px & 7) | (y & 7) << 3] - 31.5f) * 0.2f + 0.5f;
                    adj = ((PaletteReducer3.TRI_BLUE_NOISE_B[(px & 63) | (y & 63) << 6] + 0.5f));
                    adj = adj * strength / (12f + Math.abs(adj)) + 0.5f;
                    int rr = Math.min(Math.max((int) (adj + ((color >>> 24)       )), 0), 255);
                    adj = ((PaletteReducer3.TRI_BLUE_NOISE_C[(px & 63) | (y & 63) << 6] + 0.5f));
                    adj = adj * strength / (12f + Math.abs(adj)) + 0.5f;
                    int gg = Math.min(Math.max((int) (adj + ((color >>> 16) & 0xFF)), 0), 255);
                    adj = ((PaletteReducer3.TRI_BLUE_NOISE  [(px & 63) | (y & 63) << 6] + 0.5f));
                    adj = adj * strength / (12f + Math.abs(adj)) + 0.5f;
            ...
            for (int px = 0; px < lineLen; px++) {
                ...
                if ((color & 0x80) == 0 && hasTransparent)
                    ...
                    used = paletteArray[paletteMapping[((rr << 7) & 0x7C00)
                            | ((gg << 2) & 0x3E0)
                            | ((bb >>> 3))] & 0xFF];
                    adj = ((PaletteReducer3.TRI_BLUE_NOISE[(px & 63) | (y & 63) << 6] + 0.5f) * 0.007843138f);
                    adj *= adj * adj;
    ...
    /**
     * Modifies the given Pixmap so it only uses colors present in this PaletteReducer3, using Floyd-Steinberg to dither
        ...
        for (int y = 0; y < h; y++) {
            ...
            for (int px = 0; px < lineLen; px++) {
                ...
                if ((color & 0x80) == 0 && hasTransparent)
                    ...
                    float tbn = PaletteReducer3.TRI_BLUE_NOISE_MULTIPLIERS[(px & 63) | ((y << 6) & 0xFC0)];
...
//                    float b1 = bdiff * 16f / (float)Math.sqrt(2048f + bdiff * bdiff);
    ...
    /**
     * Modifies the given Pixmap so that it only uses colors present in this PaletteReducer3, dithering when it can
    ...
    {
        for (int i = 0; i < 16; i++) {
            for (int j = i; j > 0 && (paletteLightness[i16[j - 1]] > paletteLightness[i16[j]]); j--) {
                swap(i16, j, j - 1);
            }
        }
    }
     ...
     * late 2019. The output this produces is very dependent on the palette and this PaletteReducer3's dither strength,
     ...
     * The output this produces is very dependent on the palette and this PaletteReducer3's dither strength, which can be
    ...
    /**
     * Modifies {@code color} so its RGB values will match the closest color in this PaletteReducer3's palette. If color
    ...
    /**
     * Edits this PaletteReducer3 by changing each used color in the Oklab color space with an {@link Interpolation}.
     ...
     * @return this PaletteReducer3, for chaining
     */
    public PaletteReducer3 alterColorsLightness(Interpolation lightness) {
     ...
     * @return this PaletteReducer3, for chaining
     */
    public PaletteReducer3 alterColorsOklab(Interpolation lightness, Interpolation greenToRed, Interpolation blueToYellow) {
        int[] palette = paletteArray;
        for (int idx = 0; idx < colorCount; idx++) {
            int s = shrink(palette[idx]);
    ...
    /**
     * Edits this PaletteReducer3 by changing each used color so lighter colors lean towards warmer hues, while darker
     * colors lean toward cooler or more purple-ish hues.
     * @return this PaletteReducer3, for chaining
     */
    public PaletteReducer3 hueShift() {
        int[] palette = paletteArray;
        for (int idx = 0; idx < colorCount; idx++) {
            int s = shrink(palette[idx]);
            float L = OKLAB[0][s];
