...
public class OpenSimplex2H {
	...
	private double noise2_Base(double xs, double ys) {
		...
		for (int i = 0; i < 3; i++) {
			...
			int pxm = (xsb + c.xsv), pym = (ysb + c.ysv);
			Grad2 grad = GRADIENTS_2D[hash(pxm, pym)];
	...
	private double noise3_BCC(double xr, double yr, double zr) {
		...
		while (c != null) {
			...
			if (attn < 0) {
				...
				int pxm = (xrb + c.xrv), pym = (yrb + c.yrv), pzm = (zrb + c.zrv);
				Grad3 grad = GRADIENTS_3D[hash(pxm, pym, pzm)];
	...
	private double noise4_Base(double xs, double ys, double zs, double ws) {
		...
		for (int i = 0; i < 5; i++) {
			...
			if (attn > 0) {
				Grad4 grad = GRADIENTS_4D[hash(xsb, ysb, zsb, wsb)];
	...
	static {
		...
		for (int i = 0; i < GRADIENTS_2D.length; i++) {
			GRADIENTS_2D[i].dx /= N2;
			GRADIENTS_2D[i].dy /= N2;
		}
		
		GRADIENTS_3D = new Grad3[] {
		...
		for (int i = 0; i < GRADIENTS_3D.length; i++) { 
			GRADIENTS_3D[i].dx /= N3;
			GRADIENTS_3D[i].dy /= N3;
			GRADIENTS_3D[i].dz /= N3;
		}
		
		GRADIENTS_4D = new Grad4[] {
		...
		for (int i = 0; i < GRADIENTS_4D.length; i++) {
			GRADIENTS_4D[i].dx /= N4;
			GRADIENTS_4D[i].dy /= N4;
			GRADIENTS_4D[i].dz /= N4;
			GRADIENTS_4D[i].dw /= N4;
		}
	}
