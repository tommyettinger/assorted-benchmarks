...
public final class SwissTable<K, V> extends AbstractMap<K, V> {
    ...
    private static final long LONG_PHI_MAGIC = 0x9E3779B97F4A7C15L;
    ...
    private static final byte DELETED_CONTROL = -128;
    ...
    public static class Entry<K, V> implements Map.Entry<K, V> {
        ...
        public V setValue(V value) {
            throw new UnsupportedOperationException("setValue() not supported on SwissTable.Entry");
        }
    }
    public static class EntrySet<K, V> extends AbstractSet<Map.Entry<K, V>> implements Iterator<Map.Entry<K, V>>{

        private SwissTable<K, V> swiss;
        ...
        public EntrySet(SwissTable<K, V> swiss) {
            this.swiss = swiss;
            slotIndex = 0L;
            iterations = 0L;
            entry = new Entry();
        ...
        public boolean hasNext() {
            return iterations < swiss.size;
        }

        @Override
        public Entry<K, V> next() {
            K k;
            Object[] table = swiss.table;
            while ((k = readKey(table, slotIndex++)) == null);
            entry.key = k;
            entry.value = readValue(table, slotIndex - 1);
            iterations++;
            return entry;
        }
