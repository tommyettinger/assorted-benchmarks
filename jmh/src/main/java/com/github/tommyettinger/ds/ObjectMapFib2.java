 ...
 */
public class ObjectMapFib2<K, V> implements Map<K, V>, Iterable<Map.Entry<K, V>>, Serializable {
	 ...
	 */
	public ObjectMapFib2(int initialCapacity) {
	 ...
	 * @param initialCapacity If not a power of two, it is increased to the next nearest power of two.
	 */
	public ObjectMapFib2(int initialCapacity, float loadFactor) {
	 ...
	 */
	public ObjectMapFib2(ObjectMapFib2<? extends K, ? extends V> map) {
	 ...
	 * Creates a new map identical to the specified map.
	 */
	public ObjectMapFib2(Map<? extends K, ? extends V> map) {
		this(map.size());
	...
	protected int place (Object item) {
		return (int)(item.hashCode() * 0x9E3779B5L) >>> shift;
	}
	...
	public void putAll (ObjectMapFib2<? extends K, ? extends V> map) {
...
//		}
	...
	public boolean equals (Object obj) {
		if (obj == this)
			return true;
		if (!(obj instanceof ObjectMapFib2))
			return false;
		ObjectMapFib2 other = (ObjectMapFib2)obj;
		if (other.size != size)
			return false;
		K[] keyTable = this.keyTable;
		V[] valueTable = this.valueTable;
	 ...
	 * does not permit nested iteration. Iterate over {@link Entries#Entries(ObjectMapFib2)} if you
	...
	static protected abstract class MapIterator<K, V, I> implements Iterable<I>, Iterator<I> {
		...
		protected final ObjectMapFib2<K, V> map;
		protected int nextIndex, currentIndex;
		protected boolean valid = true;

		public MapIterator (ObjectMapFib2<K, V> map) {
			this.map = map;
			reset();
		}
	...
	static public class Entries<K, V> extends AbstractSet<Map.Entry<K, V>> {
		...
		public Entries (ObjectMapFib2<K, V> map) {
	...
	static public class Values<V> extends AbstractCollection<V> {
		...
		public Values (ObjectMapFib2<?, V> map) {
			iter = new MapIterator<Object, V, V>((ObjectMapFib2<Object, V>)map) {
				@Override
				public Iterator<V> iterator () {
					return this;
				}
	...
	static public class Keys<K> extends AbstractSet<K> {
		...
		public Keys (ObjectMapFib2<K, ?> map) {
			iter = new MapIterator<K, Object, K>((ObjectMapFib2<K, Object>)map) {
				@Override
