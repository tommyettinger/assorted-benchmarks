...
/**
 * A second iteration of <a href="https://www.corsix.org/content/my-favourite-small-hash-table">this hash table</a>.
 * <br>
 * Constraints at this stage:
 * <ol>
 *     <li>Keys are arbitrary 32-bit integers.</li>
 ...
 * </ol>
 * Keys are mixed using one of the better 2-round functions from
 * <a href="https://github.com/skeeto/hash-prospector">hash-prospector</a>.
 */
public class IntIntTable implements Iterable<IntIntTable.Entry> {
	...
	public IntIntTable(int[] keys, int[] values, int offset, int length){
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof IntIntTable)) {
			return false;
		}
		IntIntTable other = (IntIntTable) obj;
	...
	/**
	 * Makes a String from the contents of this IntIntTable, but uses the given {@link IntAppender} and
	...
	/**
	 * Appends to a StringBuilder from the contents of this IntIntTable, but uses the given {@link IntAppender} and
	 * {@link IntAppender} to convert each key and each value to a customizable representation and append them
	...
	protected static class MapIterator {
		protected IntIntTable table;
		protected int nextIndex = -1;
		protected boolean hasNext;

		public MapIterator(IntIntTable table){
			this.table = table;
			if(table.count > 0)
				findNextIndex();
			else
	...
	public static class KeyIterator extends MapIterator implements IntIterator {
		public KeyIterator(IntIntTable table) {
			super(table);
		}

		@Override
		public int nextInt() {
			int i = (nextIndex > table.mask) ? 0 : unmix((int) table.slots[nextIndex]);
			findNextIndex();
			return i;
		}
	}
	...
	public static class KeySet implements PrimitiveSet.SetOfInt {
		...
		public KeySet(IntIntTable table) {
	...
	public static class ValueIterator extends MapIterator implements IntIterator {
		public ValueIterator(IntIntTable table) {
			super(table);
		}
	...
	public static class Values implements PrimitiveCollection.OfInt {
		...
		public Values(IntIntTable table) {
	...
	public static class EntryIterator extends MapIterator implements Iterator<Entry> {
		public EntryIterator(IntIntTable table) {
			super(table);
		}
		...
		public Entry next() {
			...
			findNextIndex();
			return new Entry(unmix((int) slot), (int)(slot >>> 32));
		}
	}
	...
	public static class EntrySet extends AbstractSet<Entry> implements EnhancedCollection<Entry> {
		...
		public EntrySet(IntIntTable table) {
	 ...
	 */
	public static IntIntTable with() {
		return new IntIntTable(0);
	}
	 ...
	 */
	public static IntIntTable with(Number key0, Number value0) {
		IntIntTable map = new IntIntTable(1);
	 ...
	 */
	public static IntIntTable with(Number key0, Number value0, Number key1, Number value1) {
		IntIntTable map = new IntIntTable(2);
		map.put(key0.intValue(), value0.intValue());
	 ...
	 * @return a new map containing the given key-value pairs
	 */
	public static IntIntTable with(Number key0, Number value0, Number key1, Number value1, Number key2, Number value2) {
		IntIntTable map = new IntIntTable(3);
		map.put(key0.intValue(), value0.intValue());
		map.put(key1.intValue(), value1.intValue());
	 ...
	 * @return a new map containing the given key-value pairs
	 */
	public static IntIntTable with(Number key0, Number value0, Number key1, Number value1, Number key2, Number value2, Number key3, Number value3) {
		IntIntTable map = new IntIntTable(4);
		map.put(key0.intValue(), value0.intValue());
		map.put(key1.intValue(), value1.intValue());
		map.put(key2.intValue(), value2.intValue());
	 ...
	 * {@link #IntIntTable(int[], int[])}, which takes all keys and then all values.
	 ...
	 * @return a new map containing the given key-value pairs
	 */
	public static IntIntTable with(Number key0, Number value0, Number... rest) {
		IntIntTable map = new IntIntTable(1 + (rest.length >>> 1));
		map.put(key0.intValue(), value0.intValue());
		map.putPairs(rest);
		return map;
	}
	 ...
	 * @return a new map containing nothing
	 */
	public static IntIntTable withPrimitive() {
		return new IntIntTable(0);
	}
	 ...
	 * @return a new map containing just the entry mapping key0 to value0
	 */
	public static IntIntTable withPrimitive(int key0, int value0) {
		IntIntTable map = new IntIntTable(1);
		map.put(key0, value0);
		return map;
	}
	 ...
	 * @return a new map containing the given key-value pairs
	 */
	public static IntIntTable withPrimitive(int key0, int value0, int key1, int value1) {
		IntIntTable map = new IntIntTable(2);
		map.put(key0, value0);
		map.put(key1, value1);
		return map;
	 ...
	 */
	public static IntIntTable withPrimitive(int key0, int value0, int... rest) {
		IntIntTable map = new IntIntTable(1 + (rest.length >>> 1));
		map.put(key0, value0);
	 ...
	 * @param keyValueSeparator the String separating every key from its corresponding value
	 */
	public static IntIntTable parse(String str,
