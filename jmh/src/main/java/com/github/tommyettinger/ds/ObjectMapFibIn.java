 ...
 */
public class ObjectMapFibIn<K, V> implements Map<K, V>, Iterable<Map.Entry<K, V>>, Serializable {
	 ...
	 */
	public ObjectMapFibIn(int initialCapacity) {
	 ...
	 * @param initialCapacity If not a power of two, it is increased to the next nearest power of two.
	 */
	public ObjectMapFibIn(int initialCapacity, float loadFactor) {
	 ...
	 */
	public ObjectMapFibIn(ObjectMapFibIn<? extends K, ? extends V> map) {
	 ...
	 * Creates a new map identical to the specified map.
	 */
	public ObjectMapFibIn(Map<? extends K, ? extends V> map) {
		this(map.size());
	...
	protected int locateKey (Object key) {
		...
		for (int i = (int)(key.hashCode() * 0x9E3779B97F4A7C15L >>> shift); ; i = i + 1 & mask) {
			K other = keyTable[i];
			if (other == null)
	...
	public void putAll (ObjectMapFibIn<? extends K, ? extends V> map) {
	...
	public @Nullable V remove (Object key) {
		...
		while ((key = keyTable[next]) != null) {
			int placement = (int)(key.hashCode() * 0x9E3779B97F4A7C15L >>> shift);
...
//		}
	...
	public boolean equals (Object obj) {
		if (obj == this)
			return true;
		if (!(obj instanceof ObjectMapFibIn))
			return false;
		ObjectMapFibIn other = (ObjectMapFibIn)obj;
		if (other.size != size)
			return false;
		K[] keyTable = this.keyTable;
		V[] valueTable = this.valueTable;
	 ...
	 * does not permit nested iteration. Iterate over {@link Entries#Entries(ObjectMapFibIn)} if you
	...
	static protected abstract class MapIterator<K, V, I> implements Iterable<I>, Iterator<I> {
		...
		protected final ObjectMapFibIn<K, V> map;
		protected int nextIndex, currentIndex;
		protected boolean valid = true;

		public MapIterator (ObjectMapFibIn<K, V> map) {
			this.map = map;
			reset();
		}
		...
		public void remove () {
			...
			K key;
			while ((key = keyTable[next]) != null) {
				int placement = (int)(key.hashCode() * 0x9E3779B97F4A7C15L >>> map.shift);
				if ((next - placement & mask) > (i - placement & mask)) {
					keyTable[i] = key;
					valueTable[i] = valueTable[next];
					i = next;
	...
	static public class Entries<K, V> extends AbstractSet<Map.Entry<K, V>> {
		...
		public Entries (ObjectMapFibIn<K, V> map) {
	...
	static public class Values<V> extends AbstractCollection<V> {
		...
		public Values (ObjectMapFibIn<?, V> map) {
			iter = new MapIterator<Object, V, V>((ObjectMapFibIn<Object, V>)map) {
				@Override
				public Iterator<V> iterator () {
					return this;
				}
	...
	static public class Keys<K> extends AbstractSet<K> {
		...
		public Keys (ObjectMapFibIn<K, ?> map) {
			iter = new MapIterator<K, Object, K>((ObjectMapFibIn<K, Object>)map) {
				@Override
