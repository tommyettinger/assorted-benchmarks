 ...
 */
public class ObjectMapChanging<K, V> implements Map<K, V>, Iterable<Map.Entry<K, V>> {
...
//	protected long randomAddend = 0x9E3779B97F4A7C15L;
	 ...
	 */
	public ObjectMapChanging(int initialCapacity) {
	 ...
	 */
	public ObjectMapChanging(int initialCapacity, float loadFactor) {
	 ...
	 *
	 * @param map an ObjectMapChanging to copy
	 */
	public ObjectMapChanging(ObjectMapChanging<? extends K, ? extends V> map) {
	 ...
	 * Creates a new map identical to the specified map.
	 *
	 * @param map a Map to copy; ObjectMapChanging or its subclasses will be faster
	 */
	public ObjectMapChanging(Map<? extends K, ? extends V> map) {
		this(map.size());
		for (K k : map.keySet()) {
			put(k, map.get(k));
		}
	 ...
	 */
	public ObjectMapChanging(K[] keys, V[] values) {
	 ...
	 */
	public ObjectMapChanging(Collection<? extends K> keys, Collection<? extends V> values) {
...
//		return item.hashCode() & mask;
	 ...
	 */
	public void putAll (ObjectMapChanging<? extends K, ? extends V> map) {
	 ...
	 * <br>
	 * Note that {@link #putAll(ObjectMapChanging)} is more specific and can be
...
//		randomMultiplier = MathTools.GOLDEN_LONGS[(int) (randomMultiplier * size >>> 54)];
	...
	public boolean equals (Object obj) {
		...
		if (!(obj instanceof ObjectMapChanging)) {return false;}
		ObjectMapChanging other = (ObjectMapChanging)obj;
	 ...
	 * does not permit nested iteration. Iterate over {@link Entries#Entries(ObjectMapChanging)} if you
	 * need nested or multithreaded iteration. You can remove an Entry from this ObjectMapChanging
	 * using this Iterator.
	 *
	...
	static protected abstract class MapIterator<K, V, I> implements Iterable<I>, Iterator<I> {
		...
		protected final ObjectMapChanging<K, V> map;
		protected int nextIndex, currentIndex;
		protected boolean valid = true;

		public MapIterator (ObjectMapChanging<K, V> map) {
			this.map = map;
			reset();
		}
	...
	public static class Entries<K, V> extends AbstractSet<Map.Entry<K, V>> {
		...
		public Entries (ObjectMapChanging<K, V> map) {
	...
	public static class Values<K, V> extends AbstractCollection<V> {
		...
		public Values (ObjectMapChanging<K, V> map) {
	...
	public static class Keys<K, V> extends AbstractSet<K> {
		...
		public Keys (ObjectMapChanging<K, V> map) {
	 ...
	 */
	public static <K, V> ObjectMapChanging<K, V> with (K key0, V value0) {
		ObjectMapChanging<K, V> map = new ObjectMapChanging<>(1);
		map.put(key0, value0);
		return map;
	}
	 ...
	 * {@link #ObjectMapChanging(Object[], Object[])}, which takes all keys and then all values.
	...
	public static <K, V> ObjectMapChanging<K, V> with (K key0, V value0, Object... rest) {
		ObjectMapChanging<K, V> map = new ObjectMapChanging<>(1 + (rest.length >>> 1));
		map.put(key0, value0);
