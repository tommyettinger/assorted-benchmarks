 ...
 * return measurements in turns, the now-available {@link #COS_TABLE} and {@link #COS_TABLE_D}, and double variants in
 ...
 * on those, and double variants on all of these. The smoother variants actually do use the same {@link #COS_TABLE} that
 ...
 * not use {@link #COS_TABLE}, while tanSmoother() does, and this may be relevant if the table is not in-cache.
 * <br>
 ...
 * {@link #COS_TABLE the sine table} (or {@link #COS_TABLE_D the sine table for doubles}) that corresponds to the result
 * of sin(). That index can be used both to look up the sine, with {@code COS_TABLE[radiansToTableIndex(angle)]}, and
 * the cosine, with {@code COS_TABLE[(radiansToTableIndex(angle) + COS_TO_SIN) & TABLE_MASK]}. Unlike in the example
...
public final class CosTools {
    ...
    /**
     * The hard-coded size of {@link #COS_TABLE} in bits; this is 14 now, and could be adjusted in the future.
     */
    public static final int TABLE_BITS = 14; // 64KB. Adjust for accuracy.
    /**
     * The size of {@link #COS_TABLE}, available separately from the table's length for convenience.
     */
    public static final int TABLE_SIZE = (1 << TABLE_BITS);

    /**
     * If you add this to an index used in {@link #COS_TABLE}, you get the result of the sine instead of the cosine.
     */
    public static final int COS_TO_SIN = -(TABLE_SIZE >>> 2);
    /**
     * The bitmask that can be used to confine any int to wrap within {@link #TABLE_SIZE}. Any accesses to
     * {@link #COS_TABLE} with an index that could be out of bounds should probably be wrapped using this, as with
     * {@code COS_TABLE[index & TABLE_MASK]}.
     */
    public static final int TABLE_MASK = TABLE_SIZE - 1;

    /**
     * Multiply by this to convert from a float angle in radians to an index in {@link #COS_TABLE} (after it is rounded to int).
     */
    public static final float radToIndex = TABLE_SIZE / PI2;
    /**
     * Multiply by this to convert from a float angle in degrees to an index in {@link #COS_TABLE} (after it is rounded to int).
     */
    public static final float degToIndex = TABLE_SIZE / 360f;
    /**
     * Multiply by this to convert from a float angle in turns to an index in {@link #COS_TABLE} (after it is rounded to int).
     */
    public static final float turnToIndex = TABLE_SIZE;

    /**
     * Multiply by this to convert from a double angle in radians to an index in {@link #COS_TABLE} (after it is rounded to int).
     */
    public static final double radToIndexD = TABLE_SIZE / PI2_D;
    /**
     * Multiply by this to convert from a double angle in degrees to an index in {@link #COS_TABLE} (after it is rounded to int).
     */
    public static final double degToIndexD = TABLE_SIZE / 360.0;
    /**
     * Multiply by this to convert from a double angle in turns to an index in {@link #COS_TABLE} (after it is rounded to int).
     */
    ...
    static {
        for (int i = 0; i < TABLE_SIZE; i++) {
            COS_TABLE[i] = (float) (COS_TABLE_D[i] = Math.cos(((double)i) / TABLE_SIZE * PI2_D));
        }
    ...
    /**
     * Converts {@code radians} to an index that can be used in {@link #COS_TABLE} or {@link #COS_TABLE_D} to
     * obtain the cosine of the given angle. You can get the sine by running
     * {@code ((radiansToTableIndex(radians) + COS_TO_SIN) & TABLE_MASK)} . This method can be useful if you have
     * one angle and want to get both the sine and cosine of that angle (called the "sincos()" function elsewhere).
     * This tries to round the given angle to the nearest table index, like {@link #cos(float)} does.
     *
     * @param radians an angle in radians; may be positive or negative
     * @return the index into {@link #COS_TABLE} or {@link #COS_TABLE_D} of the cosine of radians
     */
    public static int radiansToTableIndex(final float radians) {
        return ((int)(Math.abs(radians) * radToIndex + 0.5f)) & TABLE_MASK;
    }

    /**
     * Converts {@code degrees} to an index that can be used in {@link #COS_TABLE} or {@link #COS_TABLE_D} to
     * obtain the sine of the given angle. You can get the cosine by running
     * {@code ((degreesToTableIndex(degrees) + COS_TO_SIN) & TABLE_MASK)} . This method can be useful if you have
     * one angle and want to get both the sine and cosine of that angle (called the "sincos()" function elsewhere).
     ...
     * @return the index into {@link #COS_TABLE} or {@link #COS_TABLE_D} of the sine of degrees
     */
    ...
    /**
     * Converts {@code turns} to an index that can be used in {@link #COS_TABLE} or {@link #COS_TABLE_D} to
     * obtain the sine of the given angle. You can get the cosine by running
     * {@code ((turnsToTableIndex(turns) + COS_TO_SIN) & TABLE_MASK)} . This method can be useful if you have
     * one angle and want to get both the sine and cosine of that angle (called the "sincos()" function elsewhere).
     ...
     * @return the index into {@link #COS_TABLE} or {@link #COS_TABLE_D} of the sine of turns
     */
...
//        return x * ((1.0/945.0) * x4 - (1.0/9.0) * x2 + 1.0) / ((1.0/63.0) * x4 - (4.0/9.0) * x2 + 1.0);
    ...
    public static float sinDeg(final float degrees) {
        ...
        return COS_TABLE[(idx + (idx >> 31)) & TABLE_MASK];
    }
    ...
    public static float cosDeg(final float degrees) {
        final int idx = (int)(degrees * degToIndex + 0.5f);
        return COS_TABLE[(idx + (idx >> 31) + COS_TO_SIN) & TABLE_MASK];
    }
    ...
    public static double sin(final double radians) {
        ...
        return COS_TABLE_D[(idx + (idx >> 31)) & TABLE_MASK];
    }
    ...
    public static double cos(final double radians) {
        final int idx = (int)(radians * radToIndexD + 0.5);
        return COS_TABLE_D[(idx + (idx >> 31) + COS_TO_SIN) & TABLE_MASK];
    }
    ...
    public static double tan(double radians) {
        radians *= CosTools.PI_INVERSE_D;
        radians += 0.5;
        radians -= Math.floor(radians);
        radians -= 0.5;
        radians *= CosTools.PI_D;
...
//        return x * ((1.0/945.0) * x4 - (1.0/9.0) * x2 + 1.0) / ((1.0/63.0) * x4 - (4.0/9.0) * x2 + 1.0);
     ...
     * outputs are possible from {@link CosTools#sin(float)}, and about half of those are duplicates, so if you need
    ...
    public static float sinSmooth(float radians) {
        ...
        //Maximum rel. error:      1.0000000000
        radians = radians * (CosTools.PI_INVERSE * 2f);
     ...
     * outputs are possible from {@link CosTools#cos(float)}, and about half of those are duplicates, so if you need
    ...
    public static float cosSmooth(float radians) {
        ...
        radians = radians * (CosTools.PI_INVERSE * 2f) + 1f;
        final int ceil = (int) Math.ceil(radians) & -2;
        radians -= ceil;
        final float x2 = radians * radians, x3 = radians * x2;
        return (((11 * radians - 3 * x3) / (7 + x2)) * (1 - (ceil & 2)));
    ...
    public static double sinSmooth(double radians) {
        ...
        radians = radians * (CosTools.PI_INVERSE_D * 2.0);
    ...
    public static double cosSmooth(double radians) {
        //
        radians = radians * (CosTools.PI_INVERSE_D * 2.0) + 1.0;
        final long ceil = (long) Math.ceil(radians) & -2L;
        radians -= ceil;
        final double x2 = radians * radians, x3 = radians * x2;
        return (((11 * radians - 3 * x3) / (7 + x2)) * (1 - (ceil & 2)));
     ...
     * outputs are possible from {@link CosTools#sinDeg(float)}, and about half of those are duplicates, so if you need
     ...
     * outputs are possible from {@link CosTools#cosDeg(float)}, and about half of those are duplicates, so if you need
     ...
     * outputs are possible from {@link CosTools#sinTurns(float)}, and about half of those are duplicates, so if you
     ...
     * outputs are possible from {@link CosTools#cosTurns(float)}, and about half of those are duplicates, so if you
...
//        //Maximum rel. error:      2.4425818920
...
//        return from + (to - from) * (radians - floor);
     ...
     * <br>
     * Internally, this uses the same {@link #COS_TABLE} that {@link #sin(float)} and {@link #cos(float)} use, but
    ...
    public static float tanSmoother(float radians) {
        ...
        final int maskedC = floor + COS_TO_SIN & TABLE_MASK;
        final float fromS = COS_TABLE[maskedS], toS = COS_TABLE[maskedS+1];
        final float fromC = COS_TABLE[maskedC], toC = COS_TABLE[maskedC+1];
     ...
     * <br>
     * Internally, this uses the same {@link #COS_TABLE_D} that {@link #sin(double)} and {@link #cos(double)} use, but
     * interpolates between adjacent entries in the table, rather than just using one entry for each unmodified. It
     * simply gets the sine and cosine at about the same time, then divides sine by cosine. This is different from how
    ...
    public static double tanSmoother(double radians) {
        ...
        final int maskedS = floor & TABLE_MASK;
        final int maskedC = floor + COS_TO_SIN & TABLE_MASK;
        final double fromS = COS_TABLE_D[maskedS], toS = COS_TABLE_D[maskedS+1];
        final double fromC = COS_TABLE_D[maskedC], toC = COS_TABLE_D[maskedC+1];
        return (fromS + (toS - fromS) * (radians - floor))/(fromC + (toC - fromC) * (radians - floor));
    }
     ...
     * <br>
     * Internally, this uses the same {@link #COS_TABLE} that {@link #sinDeg(float)} and {@link #cosDeg(float)} use, but
     * interpolates between adjacent entries in the table, rather than just using one entry for each unmodified. It
     * simply gets the sine and cosine at about the same time, then divides sine by cosine. This is different from how
     ...
     * <br>
     * Internally, this uses the same {@link #COS_TABLE_D} that {@link #sinDeg(double)} and {@link #cosDeg(double)} use,
     ...
     * <br>
     * Internally, this uses the same {@link #COS_TABLE} that {@link #sinTurns(float)} and {@link #cosTurns(float)} use, but
     * interpolates between adjacent entries in the table, rather than just using one entry for each unmodified. It
     * simply gets the sine and cosine at about the same time, then divides sine by cosine. This is different from how
     ...
     * <br>
     * Internally, this uses the same {@link #COS_TABLE_D} that {@link #sinTurns(double)} and {@link #cosTurns(double)} use,
     * but interpolates between adjacent entries in the table, rather than just using one entry for each unmodified. It
     * simply gets the sine and cosine at about the same time, then divides sine by cosine. This is different from how
