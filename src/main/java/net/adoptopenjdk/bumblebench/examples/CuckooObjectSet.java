 ...
 * @author Nathan Sweet */
public class CuckooObjectSet<T> implements Iterable<T> {
	...
	private CuckooObjectSetIterator iterator1, iterator2;

	/** Creates a new set with an initial capacity of 51 and a load factor of 0.8. */
	public CuckooObjectSet() {
		this(51, 0.8f);
	}

	/** Creates a new set with a load factor of 0.8.
	 * @param initialCapacity If not a power of two, it is increased to the next nearest power of two. */
	public CuckooObjectSet(int initialCapacity) {
	 ...
	 * @param initialCapacity If not a power of two, it is increased to the next nearest power of two. */
	public CuckooObjectSet(int initialCapacity, float loadFactor) {
...
//		hashShift = 31 - Integer.numberOfTrailingZeros(capacity);
	...
	public CuckooObjectSet(CuckooObjectSet<? extends T> set) {
	...
	public void addAll (CuckooObjectSet<T> set) {
...
//			resize(capacity << (stashCapacity & 1), (stashCapacity + (stashCapacity >> 1) & -2) ^ (1 - (stashCapacity & 1)));
	...
	public T first () {
		...
		throw new IllegalStateException("CuckooObjectSet is empty.");
	}
...
//	}
	...
	public boolean equals (Object obj) {
		if (!(obj instanceof CuckooObjectSet)) return false;
		CuckooObjectSet other = (CuckooObjectSet)obj;
	 ...
	 * {@link CuckooObjectSetIterator} constructor for nested or multithreaded iteration. */
	public CuckooObjectSetIterator<T> iterator () {
		if (Collections.allocateIterators) return new CuckooObjectSetIterator(this);
		if (iterator1 == null) {
			iterator1 = new CuckooObjectSetIterator(this);
			iterator2 = new CuckooObjectSetIterator(this);
		}
		if (!iterator1.valid) {
			iterator1.reset();
	...
	static public <T> CuckooObjectSet<T> with (T... array) {
		CuckooObjectSet set = new CuckooObjectSet();
		set.addAll(array);
		return set;
	}

	static public class CuckooObjectSetIterator<K> implements Iterable<K>, Iterator<K> {
		public boolean hasNext;

		final CuckooObjectSet<K> set;
		int nextIndex, currentIndex;
		boolean valid = true;

		public CuckooObjectSetIterator (CuckooObjectSet<K> set) {
			this.set = set;
			reset();
		}
		...
		public CuckooObjectSetIterator<K> iterator () {
			return this;
		}
